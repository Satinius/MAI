...
int main(int argc, char * argv[])
{
    ...
    if (argc < 2 || argc > 6) {
            ...
            do {
                ...
                switch (command) {
                    ...
                    case 1: {
                        ...
                        printf("With regard to the registry?\n");
                        scanf("%3s", opt);
                        if (!strcmp(opt, "yes")) {
                            searchWithRegistry(fd, fileStat.st_size, size, strToSearch, strlen(strToSearch));
                        } else if (!strcmp(opt, "no")) {
                            searchWithoutRegistry(fd, fileStat.st_size, size, strToSearch, strlen(strToSearch));
                        } else {
                    ...
                    case 2: {
                        menu(2);
                        int32_t leftIdx, rightIdx;
                        scanf("%d", &leftIdx);
                        scanf("%d", &rightIdx);
                        print(fd, fileStat.st_size, size, leftIdx, rightIdx);
                        printf("\n");
                        break;
                    }
                    ...
                    case 6: {
                        printf("CRC32 is %u\n", crc32(fd, fileStat.st_size, size));
                        break;
                    }
                    default: {
                        menu(5);
        ...
        if (!strcmp(argv[3], "-print")) {
            ...
            int32_t leftIdx = atoi(argv[4]);
            int32_t rightIdx = atoi(argv[5]);
            size = atoi(argv[2]);
            checkLimit(&size);
            print(fd, fileStat.st_size, size, leftIdx, rightIdx);
            printf("\n");
        ...
        if (!strcmp(argv[3], "-search")) {
            ...
            size = atoi(argv[2]);
            checkLimit(&size);
            if (!strcmp(argv[5], "withReg")) {
                searchWithRegistry(fd, fileStat.st_size, size, argv[4], strlen(argv[4]));
            } else if (!strcmp(argv[5], "withoutReg")) {
                searchWithoutRegistry(fd, fileStat.st_size, size, argv[4], strlen(argv[4]));
            } else {
                printf("Unknown command.\n");;
            }
            if (flock(fd, LOCK_UN)) {
